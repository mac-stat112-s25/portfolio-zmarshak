...
ggplot(weather, aes(y = temp3pm, x = temp9am, color = location)) + 
  geom_point() + 
  labs(x = "Temperature at 9AM (celcius)", y = "Temperature at 3 PM (celcius)", title = "Temperature in Australian Cities in the Morning vs the Afternoon", color = "Location")  
```
...
```{r}
#| fig-cap: "Temperature in Australian Cities in the morning compared to the afternoon"

ggplot(weather, aes(y = temp3pm, x = temp9am, color = location)) + 
  geom_point() + 
  labs(x = "Temperature at 9 AM (celcius)", y = "Temperature at 3 PM(celcuis)", color = "City")  
```
...
```{r}
#| fig-cap: "Density plots of 3pm temperatures in 3 Australian locations."
#| fig-alt: "A density plot of 3 pm temperatures in degrees Celcius at Hobart, Uluru, and Wollongong, Australia. Hobart has the lowest typical temperatures, while Uluru has the highest typical temperatures. Data source https://mac-stat.github.io/data/weather_3_locations.csv"
...
-   Summarize what you learn. What impact might our color choices have on one's ability to interpret the viz?
  - It's important to have highly contrasting colors, both between categories and the background. Also,  green and blue and green and red can look similar in some instances
...
At a minimum, our data viz should not mislead. Reconsider the climate change example from above. Why is this plot unethical and what impact might it have on policy, public opinion, etc?
  ...
  - It covers such a large amount of time that it appears there is no change however if you look in closely there is change and even small amount of climate change is harmful to the planet. This graph could make people believe climate change is not real.
...
-   An [article by Allen Hillery](https://dataliteracy.com/web-du-bois-story-of-resilience/) (@AlDatavizguy).
  ...
  - The graphs in this article are a great portrayal of this person's emotions and feelings as they are documenting their personal experience with long covid. It also brings visibilty to the effects of long covid on people which many people may not know.
  - I think the bar graphs in the article are nice looking. The bar chart is ok however there is a lot of data to interpret that at times it can be hard to read and understand
\
\
...
**Example**: What stands out in this example image? This is originally from C. Ware, *Information Visualization: Perception for Design*, 2004? Source: S. Few, *Now You See It*, 2009, p. 33.
  ...
  - The reapting pattern stands out
