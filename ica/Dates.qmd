...
Explain why we should only use `na.omit()` in extreme circumstances.
  ...
  - Because it may remove data relevant to your research question 
\
\
\
\
...
# month(today) outputs the numeric value of the month while month(today, label = TRUE) outputs the abbreviate name of the month
month(today)
...
```{r}
# What do these lines produce / what's their difference?
# mday is day of the month and yday is day of the year
mday(today)
...
```{r}
# What do these lines produce / what's their difference?
#wday() produce the numeric value for day of the week and wday(label = TRUE) outputs an abbreviated label of the wrrk
wday(today)
...
# today is on or after 2/14/2024
today < ymd("2024-02-14")
#today is not before 2/14/2024
```
...
```{r}
ggplot(daily_births, aes(x = date, y = total_births, color = week_day))+
...
Birthdays |>
  ...
  geom_boxplot()
#Births are typically higher in the summer months than the winter months
```
...
Birthdays |>
  ...
  geom_line()
#The more populous states had higher births per week, those being outliers, however mpre generally births somewhat peaked in the summer weeks
```
...
Birthdays |>
  ...
  ggplot(aes(x = week, y = births_per_week, color = state))+
  geom_line()
#There are some what similar however MN didn't have the as big as a dip between weeks 10 and 20 it acutally generally continued to rise
```
...
```{r}
Birthdays |>
  filter(date >= ymd("1978-09-09") & date <= ymd("1979-09-12")) |>
  group_by(state) |>
  summarize(tot_births = sum(births))|>
