...
```{r}
# we have a basic map without any of our plots
leaflet(data = fave_places) |> 
  addTiles()
```

```{r}
# we have have a map that has our cordinates ploted
...
Keep zooming -- what level of detail can you get into? Youcan can get basic street level information
How does that detail depend upon where you try to zoom in (thus what are the limitations of this tool)?
In some place you can see building outlines while in other places you can't, depends on the data provided

#### Exercise 2: Details {.unnumbered}

We can change all sorts of details in leaflet maps.
# Check if gplots is already installed. If not, install it with a mirror specified.
if(!require(gplots)){ # Use require() for conditional loading and installation
  chooseCRANmirror() # Or set it directly:
```{r}
...
What `geom` do we use for choropleth maps? `geom_map`
